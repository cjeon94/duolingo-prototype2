...
const harderExercises: HarderExercise[] = [
  ...
  },
  {
    ...
    spanish:
      "¿Podrías ayudarme a encontrar la estación de metro más cercana?",
    list: [
      "¿Podrías",
      "ayudarme",
      "a",
...
export default function Screen7({ onResponse }: Screen7Props): JSX.Element {
  ...
  const [currentHarderExercise, setCurrentHarderExercise] =
    React.useState<HarderExercise>(() =>
  ...
  React.useEffect(() => {
    if (showHarderQuiz) {
      setAvailableWords(
        [...currentHarderExercise.list].sort(() => Math.random() - 0.5)
      );
    } else {
      setAvailableWords(
        [...currentExercise.list].sort(() => Math.random() - 0.5)
      );
    }
    setSelectedWords([]);
    setIsChecked(false);
  ...
  React.useEffect(() => {
    if (!showModal) {
      const timer = setTimeout(() => {
        const sentence = showHarderQuiz
          ? currentHarderExercise.english
  ...
  const handleCheck = () => {
    ...
    const correctAnswer = showHarderQuiz
      ? currentHarderExercise.spanish
    ...
    if (isCorrect) {
      const correctSound = new Audio(
        "https://raw.githubusercontent.com/cjeon94/duolingo-sound-assets/main/Voicy_Correct%20answer%20sound%20effect.mp3"
      );
      ...
      const incorrectSound = new Audio(
        "https://raw.githubusercontent.com/cjeon94/duolingo-sound-assets/main/Voicy_Bad%20answer.mp3"
      );
  ...
  const currentSentence = showHarderQuiz
    ? currentHarderExercise.english
    : currentExercise.english;
  const currentCorrectAnswer = showHarderQuiz
    ? currentHarderExercise.spanish
    : currentExercise.spanish;
    ...
    <div className="min-h-screen bg-white">
      <div className="relative w-full min-h-screen bg-white overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-6 mb-8">
          <div
            className={`w-8 h-8 rounded-full flex items-center justify-center ${
              showHarderQuiz ? "bg-[#ff6b35]" : "bg-[#ce82ff]"
            }`}
          >
          ...
          <span
            className={`font-bold text-sm tracking-wider ${
              showHarderQuiz ? "text-[#ff6b35]" : "text-[#ce82ff]"
            }`}
          >
        ...
        <div className="px-4 pb-32">
          ...
          {/* Destination Row */}
          <div className="mb-8">
            <div className="min-h-[60px] sm:min-h-[80px] border-2 border-dashed border-[#e4e4e4] rounded-lg p-3 sm:p-4 bg-gray-50">
            ...
            <div className="flex flex-wrap gap-2">
              {availableWords.map((word, index) => (
                <button
                  ...
                  className="rounded-[15px] border border-[#e4e4e4] bg-white shadow-[0_3px_0_#e4e4e4] px-2 sm:px-3 py-2 text-xs sm:text-sm active:translate-y-[2px] active:shadow-none transition-all hover:bg-gray-50"
                >
                  {word}
                </button>
              ))}
          ...
          {result === "correct" && (
            <div className="mb-4 p-3 bg-green-100 border border-green-300 rounded-lg">
              <span className="text-green-700 font-semibold">
                {showHarderQuiz
                  ? "¡Excelente! Perfect advanced translation!"
                  : "¡Correcto! Great job!"}
              </span>
            </div>
          )}
        ...
        {/* Footer Bar */}
        <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 z-10 safe-area-bottom">
          <div className="flex gap-4">
            ...
            <button
              ...
              style={{
                backgroundColor:
                  selectedWords.length > 0 ? "#2ec748" : "#86efac",
                boxShadow:
                  selectedWords.length > 0
                    ? "0 3px 0 #27aa3d"
                    : "0 3px 0 #6ee7b7",
        ...
        {/* Home Indicator */}
        <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 z-10 hidden sm:block">
